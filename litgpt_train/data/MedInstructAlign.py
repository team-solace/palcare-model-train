...
class MedInstructAlign(DataModule):
    ...
    download_dir: Path = Path("./data/med_instruct_align")
    """The directory in which the downloaded dataset gets saved."""
    repo_id: str = "lemousehunter/med-instruct-align-1_parquet"
    ...
    def prepare_data(self) -> None:
        ...
        load_dataset(self.repo_id, cache_dir=self.download_dir.absolute().as_posix())

    def setup(self, stage: str = "") -> None:
        from datasets import load_dataset

        dataset_dict: DatasetDict = load_dataset(self.repo_id)

        train_data = format_dataset(dataset_dict['train'])
        test_data = format_dataset(dataset_dict['test'])
